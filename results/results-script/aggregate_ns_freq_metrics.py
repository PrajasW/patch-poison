...
def build_table(root: Path) -> List[Dict[str, str]]:
    # key: (scene, block_size) -> lists of run metrics
    grouped: Dict[Tuple[str, int], Dict[str, List[float]]] = {}

    pattern = re.compile(r"block_size_(\d+)")
    for rq in root.glob("*/block_size_*/exp_run_*/benchmark_recon_quality.log"):
        scene = rq.parts[-4]
        block_dir = rq.parts[-3]
        m = pattern.fullmatch(block_dir)
        if not m:
            continue
    ...
    for (scene, block_size) in sorted(grouped.keys(), key=lambda x: (x[0], x[1])):
        g = grouped[(scene, block_size)]
...
def write_markdown(rows: List[Dict[str, str]], out_md: Path) -> None:
    ...
    with out_md.open("w", encoding="utf-8") as f:
        ...
        for r in rows:
            f.write(
                "| {scene} | {block_size} | {n_runs} | {pr_psnr} | {pr_ssim} | {pr_lpips} | {gp_psnr} | {gp_ssim} | {gp_lpips} |\n".format(
                    **r
                )
            )
...
def write_latex(rows: List[Dict[str, str]], out_tex: Path) -> None:
    ...
    colspec = "llcccccc"  # Block, Scene, then 6 metrics
    ...
    with out_tex.open("w", encoding="utf-8") as f:
        f.write("% Auto-generated by aggregate_ns_freq_metrics.py\n")
        ...
        for r in rows:
            block = latex_escape(r["block_size"])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate ns_freq metrics from benchmark_recon_quality.log and "
    ...
    parser.add_argument(
        "--ns-freq-dir",
        default="ns_freq",
        help="Path to ns_freq directory (default: ns_freq)",
    )
    parser.add_argument(
        "--out-csv",
        default="results_freq/ns_freq_block_metrics_mean_std.csv",
        help="Output CSV path",
    )
    parser.add_argument(
        "--out-md",
        default="results_freq/ns_freq_block_metrics_mean_std.md",
        help="Output Markdown path",
    )
    parser.add_argument(
        "--out-tex",
        default="results_freq/ns_freq_block_metrics_mean_std.tex",
        help="Output LaTeX tabular path",
    )
    args = parser.parse_args()

    root = Path(args.ns_freq_dir)
    if not root.exists():
