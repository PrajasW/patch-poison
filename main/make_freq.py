...
BLOCK_SIZES = [1, 2, 4, 6, 8, 10, 16, 25, 50]
...
def main() -> None:
	...
	output_root = Path("/ssd_scratch/prajas/dataset/ns_freq")
	...
	variant_total = len(valid_scenes) * len(BLOCK_SIZES)
	...
	for scene in SCENES:
		...
		for block_size in BLOCK_SIZES:
			...
			dst_scene = output_root / scene / f"block_size_{block_size}"
			dst_scene.mkdir(parents=True, exist_ok=True)
			...
			processed = copy_scene_with_patched_train(
				...
				progress_desc=f"{scene} block={block_size}",
			)
			...
			print(
				f"[{scene}] block_size_{block_size}: patched {processed} train images -> {dst_scene}"
			)
