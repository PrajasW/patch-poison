...
# Alpha values to test – applied directly to the RGB channels (no alpha blending).
# The white squares become (alpha*255, alpha*255, alpha*255) while black stays at (0,0,0).
ALPHA_VALUES = [0.001, 0.1, 0.25, 0.5, 0.75, 1]

IMAGE_EXTENSIONS = (".png", ".jpg", ".jpeg")
DEFAULT_NUM_WORKERS = 100


def create_checkerboard(size: int, block_size: int, alpha: float) -> np.ndarray:
    """Create an RGB checkerboard patch with alpha baked into the RGB values.

    The 'black' squares are always (0, 0, 0).
    The 'white' squares are (int(alpha*255), int(alpha*255), int(alpha*255)),
    i.e. the alpha is multiplied into the channel values directly.
    """
    white_value = int(round(alpha * 255))
    white_value = max(0, min(255, white_value))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate checkerboard PatchPoison datasets with varying alpha (baked into RGB)."
    )
    parser.add_argument(
        "--num_workers",
        type=int,
...
def main() -> None:
    ...
    output_root = Path("/ssd_scratch/prajas/dataset/ns_alpha")
    ...
    print("=" * 72)
    print("Generating alpha-varied checkerboard PatchPoison datasets (alpha baked into RGB)")
    print("=" * 72)
    ...
    variant_total = len(valid_scenes) * len(ALPHA_VALUES)
    ...
    for scene in SCENES:
        ...
        for alpha in ALPHA_VALUES:
            patch = create_checkerboard(
                size=PATCH_SIZE, block_size=BLOCK_SIZE, alpha=alpha
            )
            # Use a clean label for the directory name
            alpha_label = str(alpha).replace(".", "_")
            dst_scene = output_root / scene / f"alpha_{alpha_label}"
            dst_scene.mkdir(parents=True, exist_ok=True)
            ...
            processed = copy_scene_with_patched_train(
                ...
                progress_desc=f"{scene} alpha={alpha}",
            )
            ...
            white_value = int(round(alpha * 255))
            print(
                f"[{scene}] alpha_{alpha_label} (white={white_value}): "
