...
# JPEG quality levels (1-100, lower = more compression / worse quality).
JPEG_QUALITIES = [10, 25, 50, 75]
...
def process_scene_train(
    ...
    """Apply JPEG compression to all train images of one scene."""
    processed = 0
    ...
    def _process_image(image_path: Path, output_path: Path) -> bool:
        ...
        # Save back as PNG to avoid double-compression artifacts.
        return cv2.imwrite(str(output_path), compressed)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate JPEG-compression baseline datasets for all scenes."
    )
    parser.add_argument(
        "--num_workers",
        type=int,
...
def main() -> None:
    ...
    output_root = Path("/ssd_scratch/prajas/dataset/ns_jpeg_compression")
    ...
    print("=" * 72)
    print("Generating JPEG-compression baseline datasets")
    print("=" * 72)
    ...
    variant_total = len(valid_scenes) * len(JPEG_QUALITIES)
    ...
    for scene in SCENES:
        ...
        for quality in JPEG_QUALITIES:
            dst_scene = output_root / scene / f"quality_{quality}"
            dst_scene.mkdir(parents=True, exist_ok=True)
            ...
            processed = process_scene_train(
                ...
                progress_desc=f"{scene} quality={quality}",
            )
            ...
            print(
                f"[{scene}] quality_{quality}: processed {processed} train images -> {dst_scene}"
            )
